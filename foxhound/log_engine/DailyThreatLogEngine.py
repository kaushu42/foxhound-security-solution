...
class DailyThreatLogEngine:
    ...
    def _set_threat_log_id_to_data(self):
        threat_logs_from_db = self._read_threat_logs_from_db()
        self._df = self._df.withColumn('log_name', lit(
            os.path.basename(self._INPUT_THREAT_LOG)))

        @F.udf(returnType=IntegerType())
        def setThreatLogsIdUdf(x):
            return threat_logs_from_db[x]

        self._df = self._df.withColumn(
            "threat_log_id", setThreatLogsIdUdf(self._df.log_name))
    ...
    def _preprocess(self):
        ...
        self._df = self._df.withColumn("received_datetime", to_timestamp(
            self._df.received_datetime, 'yyyy/MM/dd'))
        self._df = self._df.withColumn(
            "nat_source_port", self._df["nat_source_port"].cast(IntegerType()))
        self._df = self._df.withColumn(
    ...
    def _extract_threat_logs_details_event(self):
        ...
        self._write_df_to_postgres(
            self._df, 'fh_stg_thrt_log_dtl_evnt_f', 'append')
