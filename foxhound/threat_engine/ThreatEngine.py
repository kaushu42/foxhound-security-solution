...
class ThreatEngine(object):
    def __init__(self, db_engine, input_dir):
        ...
        self._REQUIRED_COLUMNS = [
            ...
            'Source address', 'Destination address', 'Rule', 'Application',
            'Virtual System', 'Source Zone',
            'Destination Zone', 'Inbound Interface', 'Outbound Interface',
            'Log Action',
            'Repeat Count', 'Source Port', 'Destination Port', 'Flags',
            'IP Protocol', 'Action',
        ...
        self._COLUMN_HEADERS = [
            'received_datetime', 'log_type', 'threat_content_type',
            'config_version', 'source_ip', 'destination_ip',
            'firewall_rule_id', 'application', 'virtual_system',
            'source_zone', 'destination_zone', 'inbound_interface',
            'outbound_interface', 'log_action',
            'repeat_count', 'source_port', 'destination_port', 'flags',
            'ip_protocol', 'action', 'url_filename',
            'threat_content_name', 'category', 'severity',
            'direction',
            'sequence_number', 'action_flags', 'source_country',
            'destination_country', 'cpadding',
            'contenttype', 'url_idx', 'device_name', 'file_url',
            'thr_category', 'contentver', 'sig_flags'
        ]

        self._FINAL_COLUMN_HEADERS = [
            'logged_datetime', 'processed_datetime', 'log_name',
            'received_datetime', 'log_type', 'threat_content_type',
            'config_version', 'source_ip', 'destination_ip',
            'firewall_rule_id', 'application', 'virtual_system',
            'source_zone', 'destination_zone', 'inbound_interface',
            'outbound_interface', 'log_action', 'repeat_count',
            'source_port', 'destination_port', 'flags',
            'ip_protocol', 'action', 'url_filename',
            'threat_content_name', 'category', 'severity',
            'direction', 'sequence_number', 'action_flags', 'source_country',
            'destination_country', 'cpadding',
            'contenttype', 'url_idx', 'device_name',
            'file_url', 'thr_category', 'contentver', 'sig_flags'
        ]
    ...
    def _preprocess(self):
        ...
        self._df["logged_datetime"] = self._get_date_from_csv_filename(
            self._csv_filename)
        self._df["processed_datetime"] = datetime.now()
        self._df["log_name"] = self._csv_filename
        self._df["received_datetime"] = pd.to_datetime(
            self._df["received_datetime"])
    ...
    def _set_firewall_rules_id_to_data(self):
        ...
        self._df["firewall_rule_id"] = self._df['firewall_rule_id'].map(
            firewall_rules)
    ...
    def _resolve_country_from_ip(self):
        ...
        self._df['source_country'] = self._df.apply(
            lambda row: resolve_country(
                row['source_ip'],
                row["source_country"]
            ), axis=1)
        self._df['destination_country'] = self._df.apply(
            lambda row: resolve_country(
                row['destination_ip'],
    ...
    def _write_df_to_db(self):
        self._df.to_sql('core_threatlogdetail', con=self._db_engine,
