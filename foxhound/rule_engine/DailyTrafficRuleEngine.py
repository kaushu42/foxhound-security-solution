...
class DailyTrafficRuleEngine:
    def __init__(self, input_traffic_log,db_engine, spark):
        ...
        self._spark = spark
        self._REQUIRED_COLUMNS = ['Source address', 'Destination address','Rule','Application']
        self._HEADER_NAMES = ["source_address", "destination_address","firewall_rule",'application']
    ...
    def _read_tables_from_db(self):
        ...
        rules_table = self._read_table_from_postgres('fh_stg_trfc_rule_f').drop('id')
    ...
    def _extract_rules_from_traffic_log(self):
        ...
        self._df = self._df.join(firewall_rules,on=[self._df.firewall_rule == firewall_rules.name],how='left')
        self._df = self._df.withColumnRenamed('id', 'firewall_rule_id').select('firewall_rule_id','source_address','destination_address','application')
        ...
        self._df = self._df.withColumn('name',concat(col('source_address'),lit('---'),col('destination_address'),lit('---'),col('application')))
